<template>
    ...
    <div class="d-flex justify-content-between">
      <!-- Clicker à gauche -->
      <div class="youtube-clicker">
        <h2>Youtube Clicker</h2>
        <p>Cliquez pour gagner des points !</p>
        <button @click="youtubeCookie" class="btn btn-success">
          <img src="@/assets/youtube.png" alt="youtube" class="youtube-image" />
        </button>
        <p>Nombre d'argent : {{ clicks }}</p>
      </div>
  ...
  export default {
    data() {
      return {
        ...
        buildings: [
        { name: 'Like', cost: 10, quantity: 0, passiveRate: 1 }, // 1 clic par seconde
        { name: 'Vues', cost: 50, quantity: 0, passiveRate: 5 }, // 5 clics par seconde
        { name: 'Partage', cost: 100, quantity: 0, passiveRate: 10 }, // 10 clics par seconde
        { name: 'Feat', cost: 200, quantity: 0, passiveRate: 20 }, // 20 clics par seconde
        { name: 'Gros concept', cost: 500, quantity: 0, passiveRate: 50 }, // 50 clics par seconde
        { name: 'Trophé', cost: 1000, quantity: 0, passiveRate: 100 }, // 100 clics par seconde
      ],
      };
    },
    methods: {
    ...
    calculatePassiveClicks() {
      ...
      buyBuilding(building) {
        if (this.clicks >= building.cost) {
          ...
          alert('Pas assez d\'argent pour acheter ce bâtiment !');
        }
      },
      async saveData() {
        try {
